...
Public Class ClsConsultas
    ...
    Public ReadOnly Property Nome_Entidade As String
        Get
    ...
    Public ReadOnly Property Nome_Medico As String
        Get
            Return If(Cod_Veterinario = "", "", ClsEntidades.GetEntidadesByCod(Cod_Veterinario).Nome)
        End Get
    End Property
    ...
    Public ReadOnly Property Tipo_Animal
        Get
            Return If(Cod_Tipo_Animal = "", "", ClsTipos_Animais.GetTipos_AnimaisByCod(Cod_Tipo_Animal).Animal)
        End Get
    End Property
    ...
    Public ReadOnly Property sHMI As String
        Get
            Return Hour(Hora_Inicio).ToString("00") & ":" & Minute(Hora_Inicio).ToString("00")
        End Get
    End Property
    Public ReadOnly Property sHMF As String
        Get
            Return Hour(Hora_Fim).ToString("00") & ":" & Minute(Hora_Fim).ToString("00")
        End Get
    End Property
    Public ReadOnly Property Veterinario As String
        Get
            Return If(Cod_Veterinario = "", "", ClsVeterinarios.GetVeterinariosByCod(Cod_Veterinario).Nome)
        End Get
    End Property
    ...
    Sub New(pCC As String, pCE As String, pCV As String, pCS As String, pCA As String, pCod_TC As String, pCod_TA As String, pHI As Date, pHF As Date, pStat As enStatus, pOBS As String)
        ...
        Cod_Sala = pCS
        Cod_Animal = pCA
        Cod_Tipo_Consulta = pCod_TC
        Cod_Tipo_Animal = pCod_TA
        Hora_Inicio = pHI
        Hora_Fim = pHF
        Status = pStat
        Observacoes = pOBS
        Dia = ClsUtils.GetDataSH(Hora_Inicio)
        FLG_Ativo = True

    End Sub

    Sub New(pCE As String, pCV As String, pCS As String, pCA As String, pCod_TC As String, pCod_TA As String, pHI As Date, pHF As Date, pStat As enStatus, pOBS As String)

        Cod_Consulta = GetNovoCodigo()
        Cod_Veterinario = pCV
        Cod_Entidade = pCE
        Cod_Sala = pCS
        Cod_Animal = pCA
    ...
    Public Shared Function GetConsultasByDia(ByVal pDia As Date) As List(Of ClsConsultas)
    ...
    Public Shared Function GetCountConsultasSemanaAtual()
        Dim mCountConsultas As Integer = 0
        ...
        mDB.ConnectionString = My.Settings.Connection
        mCMD.Parameters.Clear()
        mCMD.Parameters.AddWithValue("@mPrimeiroDia", mPrimeiroDia)
        mCMD.Parameters.AddWithValue("@mUltimoDia", mUltimoDiA)

        mStrSQL = "SELECT COUNT "
        mStrSQL += "(Cod_Consulta) As Cod_Consulta "
        mStrSQL += "FROM Consultas "
        mStrSQL += "WHERE FLG_Ativo=1 "
        mStrSQL += "AND Dia BETWEEN @mPrimeiroDia AND @mUltimoDia"
        ...
        mCountConsultas = Convert.ToInt32(mCMD.ExecuteScalar())

        mDB.Close()

        Return mCountConsultas
    End Function

    Public Shared Function GetUltimasTresConsultas()
        Dim mListaConsultas As New List(Of ClsConsultas)
        Dim mHoje As DateTime = Date.Today
        ...
        mDB.ConnectionString = My.Settings.Connection
        mCMD.Parameters.Clear()
        mCMD.Parameters.AddWithValue("@mHoje", mHoje)

        mStrSQL = "SELECT TOP 3 "
        mStrSQL += "(Cod_Veterinario) As Cod_Veterinario,"
        mStrSQL += "(Cod_Tipo_Consulta) As Cod_Tipo_Consulta,"
        mStrSQL += "(Hora_Inicio) As Hora_Inicio,"
        mStrSQL += "(Hora_Fim) As Hora_Fim "
        mStrSQL += "FROM Consultas "
        mStrSQL += "WHERE FLG_Ativo = 1 "
        mStrSQL += "AND Dia = @mHoje "
        mStrSQL += "ORDER BY Cod_Consulta DESC"
    ...
    Public Shared Function FiltrarConsultas(ByVal pFiltro As ClsConsultas) As List(Of ClsConsultas)
        ...
        mLista = mListaConsultas.FindAll(Function(x) x.Cod_Consulta.Contains(pFiltro.Cod_Consulta))
        '-----------------------------------------------------
    ...
    Public Shared Function ModificaConsultas(ByVal pClsConsultas As ClsConsultas) As Boolean
        ...
        mCMD.Parameters.AddWithValue("@pCod_Animal", pClsConsultas.Cod_Animal.Substring(0, IIf(Len(pClsConsultas.Cod_Sala) > 5, 5, Len(pClsConsultas.Cod_Animal))))
    ...
    Public Shared Function GravaConsultas(ByVal pNovoConsultas As ClsConsultas) As Boolean
        ...
        mCMD.Parameters.AddWithValue("@pCod_Animal", pNovoConsultas.Cod_Animal.Substring(0, IIf(Len(pNovoConsultas.Cod_Sala) > 5, 5, Len(pNovoConsultas.Cod_Animal))))
