...
Public Class ClsAnimais
    ...
    Public ReadOnly Property Tipo_Animal As String
        Get
            Return If(Cod_Tipo_Animal = "", "", ClsTipos_Animais.GetTipos_AnimaisByCod(Cod_Tipo_Animal).Animal)
        End Get
    End Property
    Public Property Imagem_Animal As String                                 ' Len: 10 - SQL_Type: nvarchar
    ...
    Friend Shared Function GetNovoCodigo() As String
        ...
        Return "A" & ClsEmpresas.GetContador(enCont.Animais).ToString("000") ' <-------------- VERIFICAR O Nº de ZEROS
    ...
    Public Shared Function GetCountAnimais()
        Dim mCountAnimais As Integer = 0
        ...
        mStrSQL += "(Cod_Animal) As Cod_Animal "
        mStrSQL += "FROM Animais "
        mStrSQL += "WHERE FLG_Ativo=1"
        ...
        mCountAnimais = Convert.ToInt32(mCMD.ExecuteScalar())

        mDB.Close()

        Return mCountAnimais
    End Function

    Public Shared Function GetUltimosCincoAnimais()
        Dim mListaAnimais As New List(Of ClsAnimais)
        ...
        mStrSQL = "SELECT TOP 5"
        mStrSQL += "(Cod_Animal) As Cod_Animal,"
        mStrSQL += "(Nome_Animal) As Nome_Animal,"
        mStrSQL += "(Cod_Entidade) As Cod_Entidade,"
        mStrSQL += "(Cod_Tipo_Animal) As Cod_Tipo_Animal "
        mStrSQL += "FROM Animais "
        mStrSQL += "WHERE FLG_Ativo=1 "
        mStrSQL += "ORDER BY Cod_Animal DESC"
    ...
    Public Shared Function ModificaAnimais(ByVal pClsAnimais As ClsAnimais) As Boolean
        ...
        mCMD.Parameters.AddWithValue("@pImagem_Animal", pClsAnimais.Imagem_Animal.Substring(0, IIf(Len(pClsAnimais.Imagem_Animal) > 10, 10, Len(pClsAnimais.Imagem_Animal))))
    ...
    Public Shared Function GravaAnimais(ByVal pNovoAnimais As ClsAnimais) As Boolean
        ...
        mCMD.Parameters.AddWithValue("@pImagem_Animal", pNovoAnimais.Imagem_Animal.Substring(0, IIf(Len(pNovoAnimais.Imagem_Animal) > 10, 10, Len(pNovoAnimais.Imagem_Animal))))
